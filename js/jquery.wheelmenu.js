...
!function($){
  ...
  $.fn.flyIn = function (el, button, width, height, angle, step, radius, settings) {
    var d = 0;
    this.stop(true,true);
    var fields1 = this.length;
    this.each(function(index) {
      if(index < 3){
        var step1 = (settings.angle[1] - settings.angle[0]) / 3;
        angle = (settings.angle[0] + (step1 * index)) * (Math.PI/180); 
        var x = Math.round(width/2 + radius * Math.cos(angle)*0.6 - $(this).find("a").outerWidth()/2),
            y = Math.round(height/2 + radius * Math.sin(angle)*0.6 - $(this).find("a").outerHeight()/2);
        ...
        var step2 = (settings.angle[1] - settings.angle[0]) / (fields1 - 3);
        angle = (settings.angle[0] + (step2 * (index - 3))) * (Math.PI/180); 
