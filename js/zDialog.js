...
var $topWindow = function () {
    ...
    while (parentWin != parentWin.parent) {
        if (parentWin.parent.document.getElementsByTagName("FRAMESET").length > 0 || parentWin.parent.document.getElementsByTagName("IFRAME").length > 0 ) break;
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:14px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="20" height="80" style="background-image: url(' + IMAGESPATH + 'dialog_lt.png) !important;background: url(' + IMAGESPATH + 'dialog_lt.gif) no-repeat 0.4px 0;"><div style="width: 20px;"></div></td>\
      <td height="80" style="background-image:url(' + IMAGESPATH + 'dialog_ct.png) !important;background: url(' + IMAGESPATH + 'dialog_ct.gif) repeat-x top;"><div style="padding: 8px 0 0 8px; float: left;font-size: 1.8em; color:#fff;"><span id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn.png)\'" style="margin-top:10px; position: relative; cursor: pointer; float: right;margin-right: 15px; height: 34px; width: 34px; background-image: url(' + IMAGESPATH + 'dialog_closebtn.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
      <td width="20" height="80" style="background-image: url(' + IMAGESPATH + 'dialog_rt.png) !important;background: url(' + IMAGESPATH + 'dialog_rt.gif) no-repeat -0.6px 0;"><div style="width: 20px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td width="20" style="background-image: url(' + IMAGESPATH + 'dialog_mlm.png) !important;background: url(' + IMAGESPATH + 'dialog_mlm.gif) repeat-y 0.4px;"></td>\
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#062153">\
	...
	' + (function (obj) {
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + ';background: transparent;">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="padding: 8px 20px; text-align: center;background-color: transparent; ">\
                <input type="button" class="buttonStyle" style="width: 90px; height:36px; border: none; color: #ffffff; margin-right:15px; font-size: 14px;background: url(../images1/btn_bg.png) no-repeat -389px 0; cursor: pointer;" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="buttonStyle" style="width: 90px; height:36px; border: none; color: #ffffff; font-size: 14px;background: url(../images1/btn_bg.png) no-repeat -389px -59px; cursor: pointer;" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
      <td width="20" style="background-image: url(' + IMAGESPATH + 'dialog_mrm.png) !important;background: url(' + IMAGESPATH + 'dialog_mrm.gif) repeat-y -0.4px;"></td>\
    </tr>\
    <tr>\
      <td width="22" height="22" style="background-image: url(' + IMAGESPATH + 'dialog_lb.png) !important;background: url(' + IMAGESPATH + 'dialog_lb.gif) no-repeat 0.4px -0.4px;"></td>\
      <td style="background-image: url(' + IMAGESPATH + 'dialog_cb.png) !important;background: url(' + IMAGESPATH + 'dialog_cb.gif) repeat-x -0.6px 0;"></td>\
      <td width="22" height="22" style="background-image: url(' + IMAGESPATH + 'dialog_rb.png) !important;background: url(' + IMAGESPATH + 'dialog_rb.gif) no-repeat -0.6px -0.4px;"><a onfocus=\'$id("_forTab_' + this.ID + '").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
'   
    // var div="";
    // var pw = topWin.parentNode;
    // console.log(pw);
    // if(!pw){
    //     div = topWin.$id("_DialogDiv_" + this.ID);
    // }else {
    //     div = pw.$id("_DialogDiv_" + this.ID);
    // }
